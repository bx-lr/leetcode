...
class Solution:
    ...
    def check(self) -> int:
        ...
        '''
        arg0 = ["set", "get", "get", "set", "get", "get"]
        arg1 = [["foo", "bar", 1], ["foo", 1], ["foo", 3], ["foo", "bar2", 4], ["foo", 4], ["foo", 5]]
        expected = [None, "bar", "bar", None, "bar2", "bar2"]
        results = []
        for a0, a1, exp in zip(arg0, arg1, expected):
            if a0 == 'set':
                res = self.set(a1[0], a1[1], a1[2])
            if a0 == 'get':
                res = self.get(a1[0], a1[1])
            results.append(res)
            if res == exp:
...
def get_instructions() -> str:
    ins = '''Time Based Key-Value Store
